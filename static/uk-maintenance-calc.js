...
(function () {
  ...
  // Confirmed against GOV.UK on 2026-07-12; the server keeps these current automatically.
  var FIGURES = {
    ...
    accomCap: 1334,   // max accommodation prepayment (to the institution) you can offset
    asOf: "July 2026",
    source: "default",  // becomes "live" | "fallback" | "env" once fetched
  ...
  function dates(appDateStr) {
    if (!appDateStr) return null;
    var app = new Date(appDateStr + "T00:00:00");
    if (isNaN(app.getTime())) return null;
    var holdStart = new Date(app); holdStart.setDate(app.getDate() - 27); // 28 days incl. app date
    var earliestClose = new Date(app); earliestClose.setDate(app.getDate() - 31);
    return { app: app, holdStart: holdStart, earliestClose: earliestClose };
  }
  ...
  function applyToProse(root) {
    ...
    scope.querySelectorAll("[data-ukm]").forEach(function (el) {
      ...
      else if (FIGURES[key] != null) el.textContent = gbp(FIGURES[key]);
    });
  }

  function _applyFigures(d) {
    if (!d || typeof d !== "object") return;
    ["london", "outside", "maxMonths", "accomCap", "asOf", "source", "sourceUrl", "publicPath"].forEach(function (k) {
  ...
  function asOfHtml() {
    ...
    if (FIGURES.source === "live") {
      return '<span style="color:#16a34a;font-weight:700">✓ Live Home Office rates, verified from GOV.UK.</span> ' +
  ...
  function mount(container) {
    ...
    container.innerHTML =
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">' +
        ...
        '<div style="grid-column:1/-1">' + lbl("Intended application date (optional)") + '<input class="ukm-appDate" type="date" style="' + INP + '"></div>' +
      "</div>" +
    ...
    function recompute() {
      ...
      var d = dates(q(".ukm-appDate").value);
      var dateBox = d
        ? '<div style="margin-top:8px;font-size:12.5px;color:#3730a3;line-height:1.6">Applying on <b>' + fmtDate(d.app) + "</b>: keep at least <b>" + gbp(r.total) + "</b> in your account <b>every day</b> from <b>" + fmtDate(d.holdStart) + "</b> to <b>" + fmtDate(d.app) + "</b> (28 days), never dipping below it. Your bank statement's closing date must be on or after <b>" + fmtDate(d.earliestClose) + "</b>.</div>"
        : '<div style="margin-top:8px;font-size:12.5px;color:#3730a3;line-height:1.6">Hold at least <b>' + gbp(r.total) + "</b> for <b>28 consecutive days</b> without dipping below it, and apply within <b>31 days</b> of your statement's closing date. Add your application date above for the exact window.</div>";
    ...
    ["ukm-months", "ukm-tuition", "ukm-tuitionPaid", "ukm-accom", "ukm-appDate"].forEach(function (cls) {
